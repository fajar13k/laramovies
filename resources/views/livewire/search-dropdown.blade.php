<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
	<input
		...
		type="text"
		class="bg-gray-800 rounded-full w-64 px-4 py-1 focus:outline-none focus:shadow-outline"
		placeholder="Search"
		x-ref="search"
		@keydown.window="
						...
						<li class="border-b border-gray-700">
							...
							>
								...
								@if ($result['release_date'])
									<span class="ml-4">{{ $result['title'] . ' (' . \Carbon\Carbon::parse($result['release_date'])->format('Y') . ')' }}</span>
								@else
									<span class="ml-4">{{ $result['title'] . ' (Unknown Year)' }}</span>
								@endif
							</a>
						</li>
					@endforeach
